...
const countriesByName = () => {
    let countries = countries_data.filter(country => country.name)
...
const countriesByPopulation = () => {
    let populationSorted = countries_data.sort((a,b) => b.population - a.population)
    populationSorted.forEach(x => console.log(x))
...
const mostSpokenLanguages = (countriesArray, topN) => {
    const mostSpoken = Object.entries(
        countriesArray
            .reduce((langs, country) => {
                country.languages.forEach(lang => langs.push(lang));
                return langs;
            }, [])
            .reduce((dict, lang) => {
